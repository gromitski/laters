...
- When the maintainer explicitly requests a separate development branch, the bounded work may instead be committed and pushed there for engineering integration. A development-branch push is not a human acceptance environment when GitHub Pages publishes only `main`: complete all agent-owned checks on the branch, obtain authorisation to promote the exact tested change, then merge and push `main` before asking the maintainer to perform visual or physical-device testing.
