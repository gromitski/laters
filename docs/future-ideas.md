...
| Cross-device sync | Private Google Drive article sync is released in `v0.5.0`, with local pending operations, retained deletion records, visible-app polling, a three-state menu indicator and accepted phone/desktop add, delete and Undo propagation. | Safe remote-operation compaction remains before broader use. Detailed status remains in the drawer. This does not permit public OAuth publication. |
