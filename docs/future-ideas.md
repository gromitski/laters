...
| Cross-device sync | Private Google Drive article sync is implemented for maintainer testing, with local pending operations, retained deletion records and visible-app polling. | Physical add/delete propagation acceptance and safe remote-operation compaction remain before any broader claim. This does not permit public OAuth publication. |
