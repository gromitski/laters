...
| Cross-device sync | Private Google Drive article sync is released in `v0.5.0`, with local pending operations, visible-app polling, a three-state menu indicator and accepted phone/desktop add, delete and Undo propagation. The post-release implementation automatically checkpoints and removes covered operation files after 100 changes. | Detailed status remains in the drawer. Public OAuth publication is still a separate decision. |
...
| Export | Provides portability and a recovery route independent of optional sync. | Choose a durable, privacy-safe format and define whether re-import, duplicates and timestamps are supported. Export must be explicitly user initiated. |
