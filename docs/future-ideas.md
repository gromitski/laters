...
| Cross-device sync | Private Google Drive article sync is implemented for maintainer testing, with local pending operations, retained deletion records, visible-app polling and a three-state menu indicator. | Physical add/delete propagation acceptance and safe remote-operation compaction remain before any broader claim. Detailed status remains in the drawer. This does not permit public OAuth publication. |
